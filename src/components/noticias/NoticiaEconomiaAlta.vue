...
import { AGREGARECONOMIA } from 'src/store/noticiasEconomia/types'
...
export default {
  ...
  methods: {
    ...mapActions('noticiasEconomia', [AGREGARECONOMIA]),
    submitForm: function () {
      console.log('agregar - ABM Economia')
      this.agregarEconomia(this.economia)
      this.$q.notify({
        message: 'Noticia Agregada OK'
      })
      this.$emit('closeAltaEconomia')
    }
  }
}
</script>
